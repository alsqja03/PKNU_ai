...
# 주소 또는 키워드 → 좌표 변환 함수
def address_to_coord(keyword, kakao_api_key):
    import requests
    ...
    if documents:
        ...
        st.info(f"📍 장소명: {documents[0]['place_name']} | 주소: {documents[0]['road_address_name'] or documents[0]['address_name']}")
        return x, y

    st.error(f"❌ '{keyword}'에 대한 장소를 찾을 수 없습니다.")
    return None, None
